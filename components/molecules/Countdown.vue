...
export default Vue.extend({
  ...
  methods: {
    ...mapMutations("Countdown", {
      setTime: Mutations.SET_TIME, // renomeando mutation
      resetTime: Mutations.RESET_TIME, // renomeando mutation
    }),
  ...
  watch: {
    isActive(newValue: boolean) {
      ...
      !newValue && this.resetTime();
    },

    time(newValue: number) {
      newValue > 0 ? this.runCountdown(true) : this.$emit("completed");
    },
  },
});
</script>